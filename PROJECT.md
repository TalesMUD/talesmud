...
- **Room-Based World System**
  ...
  - Player chrome Map (not Atlas): Inventory-style centered Map overlay on `document.body` (Esc/X); never Materialize `.modal`; biome pixel tiles per room; label LOD + one you-are-here; Map chrome on action bar
