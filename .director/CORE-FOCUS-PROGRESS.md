...
## B5 — Terminal word wrap and docked hotbar
- SHA: `9681bdf5e420484f70554fe8ad4730b4c76f90a6` (`9681bdf`)
- What changed: xterm output is soft-wrapped on spaces before it is written, and the scrollback is redrawn when the column count changes. A token longer than the row can still break. ANSI color sequences stay in the stream and do not count as columns. The default preset no longer has a separate hotbar row. The spell bar is the top strip of the action bar (one frame, empty slots still dashed). A saved full-width hotbar that sat directly on the action bar is folded into that dock on load. A hotbar the player moved somewhere else stays its own widget. Cache-bust `?v=b5wrap`.
- Tests: `node public/mud-client/src/game/echo/wrapAnsi_test.mjs` and `node public/mud-client/src/game/layout/layoutPresets_test.mjs` green. mud-client `npm run build` succeeded (existing unused-CSS and a11y warnings only).
- Deploy: pushed `engine-june`. VPS fast-forwarded `fde0b44` → `9681bdf`, copied the client into `pkg/webuiplay/dist`, rebuilt `bin/tales`. `sudo systemctl restart` needs a password, so pid 723455 was SIGTERM'd and `Restart=always` started pid 754506. Door pid 695590 on :8020 was not restarted.
