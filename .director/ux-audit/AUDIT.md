...
Captured 2026-09-24 against the local demo server `http://127.0.0.1:8010/play/` (`?v=a3reward`) with a fresh guest in the Awakening Chamber. Tooling: headless Chromium via puppeteer-core (`/tmp/b1pw/audit.js`). Screenshots are in `.director/ux-audit/before/`.
...
Re-shot against the local demo server `http://127.0.0.1:8010/play/` (`?v=b4chrome`) with a fresh guest. Files are in `.director/ux-audit/after/`. Same sizes as the before set.
