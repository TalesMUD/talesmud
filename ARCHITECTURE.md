...
2. **NPC Updates** (10 seconds)
   ...
   - Execute behavior updates: idle wandering within `WanderRadius`, ordered patrol path movement, and idle dialog chatter cooldowns
   - Broadcast NPC movement/chatter messages to affected rooms
...
When talking to a quest-source NPC, quest dialog options (offer, progress check, turn-in) are automatically injected into the NPC's conversation. If the NPC has no main dialog, `talk`/`speak` creates a quest-only conversation so numbered choices still route through the dialog selection handler.
...
type NPC struct {
    *entities.Entity
    ...
    LastIdleDialog    time.Time
}
```

NPC room payloads are generated by `util.GetRoomNPCs` from live `NPCInstanceManager` data. They include combat/trade/dialog state plus `isQuestGiver`, `hasDialog`, `hasIdleDialog`, and `state` flags for the MUD client entity panel.
