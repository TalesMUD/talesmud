...
(() => {
  ...
  function renderZones() {
    ...
    for (const z of state.data.zones) {
      ...
      if (!z.fog && z.imagePlate) {
        const plateHref = absMapAsset(z.imagePlate) + '?v=map7';
        const img = el('image', {
  ...
  function poiIconHref(kind) {
    ...
    return key ? `/map/assets/icons/${key}.png?v=map7` : null;
  }
  ...
  function renderCities() {
    ...
    for (const c of cities) {
      ...
      const href = fog
        ? `/map/assets/icons/town.png?v=map7`
        : `/map/assets/icons/town.png?v=map7`;
      const img = el('image', {
        class: 'poi-icon',
        href,
  ...
  function tryWorldPlate() {
    ...
    const candidates = [
      absMapAsset(layers.worldPlate) + (layers.worldPlate ? '?v=map7' : ''),
      '/map/assets/world/world-plate.jpg?v=map7',
      absMapAsset(layers.worldPlateAlt) + (layers.worldPlateAlt ? '?v=map7' : ''),
      '/map/assets/world/world-plate-16x9.jpg?v=map7',
  ...
  async function boot() {
    const res = await fetch('/map/map-data.json?v=map7', { cache: 'no-cache' });
