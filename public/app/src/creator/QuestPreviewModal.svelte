  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-6" on:click={() => dispatch("close")}>
    ...
    <div class="w-full max-w-2xl rounded-lg border border-slate-700 bg-slate-950 shadow-2xl" on:click|stopPropagation>
      <div class="flex items-center justify-between border-b border-slate-800 px-5 py-3">
        <h2 class="text-sm font-semibold">Quest Preview</h2>
      ...
      <div class="space-y-4 p-5">
        <div>
          <div class="text-lg font-semibold">{preview?.name || "Untitled quest"}</div>
          <div class="text-xs text-slate-400">Source: {preview?.source?.type || "unspecified"}</div>
        </div>
        <div class="grid grid-cols-2 gap-3 text-xs">
          <div class="rounded-md border border-slate-800 bg-slate-900/70 p-3">
            <div class="text-slate-500">Objectives</div>
            <div class="mt-1 text-xl font-semibold">{preview?.objectivesCount || 0}</div>
          </div>
          <div class="rounded-md border border-slate-800 bg-slate-900/70 p-3">
            <div class="text-slate-500">Rewards</div>
            <div class="mt-1 text-slate-200">{preview?.rewards?.xp || 0} XP, {preview?.rewards?.gold || 0} gold</div>
          </div>
        </div>
